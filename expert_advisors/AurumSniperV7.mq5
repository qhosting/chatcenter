...
#property version   "7.40" // Version Final (Trade Reporting)
#property strict
...
int OnInit()
  {
   ...
   Print(">>> AURUM SNIPER V7.40: ONLINE (Auto: ", InpEnableAutoTrade, " | Guardian: ", InpGuardianManual, ") <<<");
...
//+------------------------------------------------------------------+
//| EVENTO TRANSACTION (Reporte de Cierres)                          |
//+------------------------------------------------------------------+
void OnTradeTransaction(const MqlTradeTransaction& trans,
                        const MqlTradeRequest& request,
                        const MqlTradeResult& result)
  {
   // Filtrar solo transacciones que agregan deals al historial (Cierres)
      ...
      if(HistoryDealSelect(deal_ticket))
        {
         ...
         // Verificar si es nuestra (o manual gestionada) y si es SALIDA (Cierre)
         bool es_nuestra = (magic == MAGIC_NUMBER) || (InpGuardianManual && magic == 0);

         if(es_nuestra && entry == DEAL_ENTRY_OUT)
           {
            ...
            long reason_code = HistoryDealGetInteger(deal_ticket, DEAL_REASON);
            string reason_str = "Manual/Desconocido";

            if(reason_code == DEAL_REASON_SL) reason_str = "Stop Loss";
            else if(reason_code == DEAL_REASON_TP) reason_str = "Take Profit";
            else if(reason_code == DEAL_REASON_CLIENT) reason_str = "Cierre Manual";

            // Enviar Reporte de Cierre
            int digits = (int)SymbolInfoInteger(_Symbol, SYMBOL_DIGITS);
            EnviarReporteCierre(total, reason_str, digits);
           }
        }
     }
  }
...
//+------------------------------------------------------------------+
//| Enviar Webhook JSON (Cierre)                                     |
//+------------------------------------------------------------------+
bool EnviarReporteCierre(double profit, string razon, int digits)
  {
   string s_prof = DoubleToString(profit, 2); // 2 decimales para dinero
   string tipo_cierre = (profit >= 0) ? "PROFIT" : "LOSS";

   // Formato JSON: {"symbol": "EURUSD", "tipo": "CIERRE", "resultado": "PROFIT", "monto": 10.50, "razon": "Take Profit"}
   string json = StringFormat(
      "{\"symbol\": \"%s\", \"tipo\": \"CIERRE\", \"resultado\": \"%s\", \"monto\": %s, \"razon\": \"%s\"}",
      _Symbol, tipo_cierre, s_prof, razon
   );
   ...
   if(res == 200)
     {
      Print("REPORTE CIERRE ENVIADO: ", json);
      return true;
     }

   Print("ERROR Webhook Cierre: ", res, " Err: ", GetLastError());
   return false;
  }
//+------------------------------------------------------------------+
