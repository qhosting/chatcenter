...
#property version   "4.03" // Versión Final (Fix TP Logic & News Guard)
#property strict
   ...
   if(zona_compra && (ask > ema_h4) && (rsi_h1 < InpRSIOversold))
     {
      ...
      // CORRECCIÓN CRÍTICA: TP se suma en compras
...
bool HayNoticia()
  {
   ...
   // Usamos String para asegurar tipo
         ...
         if(CalendarEventById(values[i].event_id, event))
           {
            ...
            if(event.importance >= 2)
              {
               // Filtro 2: Divisa Afectada
               // Usamos casting explícito (string) para evitar error "implicit conversion from unknown"
               // y StringCompare para comparación segura.
               string ev_currency = (string)event.currency;
