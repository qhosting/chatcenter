...
#property version   "4.04" // Versión Final (Fix Compatibility via Country)
#property strict
...
void OnTick()
  {
   ...
   bool zona_compra = MathAbs(ask - min_20d) <= dist_puntos; // Cerca del mínimo
   bool zona_venta  = MathAbs(bid - max_20d) <= dist_puntos; // Cerca del máximo
   ...
   if(signal_type != "NONE")
     {
      // Actualizar timestamp ANTES de enviar para prevenir bucles
      last_alert_time = bar_time;

      // Normalización de Precios (Importante para JPY, Indices, etc)
      ...
      // Enviar Webhook
      if(EnviarWebhook(signal_type, norm_entry, norm_sl, norm_tp, razon_entrada, digits))
        {
            ...
            if(event.importance >= 2)
              {
               ...
               // Si 'event.currency' no existe en esta versión de MQL5,
               // obtenemos la moneda a través del País del evento.

               MqlCalendarCountry country;
               if(CalendarCountryById(event.country_id, country))
                 {
                  // 'country.currency' es estándar en la estructura MqlCalendarCountry
                  string ev_currency = country.currency;

                  if(StringCompare(ev_currency, local_base) == 0 || StringCompare(ev_currency, local_profit) == 0)
                    {
                     Print("⚠️ NOTICIA ALTO IMPACTO: ", event.name, " [", ev_currency, "]");
                     estado_cache = true;
                     return true;
                    }
                 }
                 ...
                 {
                  // Si falla al obtener país, asumimos riesgo si es importancia alta
                  Print("⚠️ NOTICIA ALTO IMPACTO (País Desconocido): ", event.name);
                  estado_cache = true;
                  return true;
                 }
              }
...
bool EnviarWebhook(string tipo, double precio, double sl, double tp, string razon, int digits)
  {
   // Construcción del Payload JSON usando DoubleToString para precisión correcta
   string str_precio = DoubleToString(precio, digits);
   string str_sl     = DoubleToString(sl, digits);
   string str_tp     = DoubleToString(tp, digits);

   string json_payload = StringFormat(
      "{\"symbol\": \"%s\", \"tipo\": \"%s\", \"precio\": %s, \"sl\": %s, \"tp\": %s, \"razon\": \"%s\"}",
      Symbol(), tipo, str_precio, str_sl, str_tp, razon
   );
